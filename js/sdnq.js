...
console.log("★★★ sdnq.js: SDNQ LoRA Stacker V2 ★★★");

app.registerExtension({
    name: "nunchaku.sdnq_lora_stacker_v2",
    
    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "SDNQLoraStackerV2_10") {
            nodeType["@visibleLoraCount"] = { type: "number", default: 1, min: 1, max: 10, step: 1 };
        }
    },

    nodeCreated(node) {
        if (node.comfyClass !== "SDNQLoraStackerV2_10") return;
